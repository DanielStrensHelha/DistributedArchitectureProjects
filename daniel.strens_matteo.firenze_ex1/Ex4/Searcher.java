...
public class Searcher{
    ...
    public static int ParallelSearch(int toSearch, int[] Array, int NumThreads) {
        ...
        for(int i = 0; i < PART_COUNT; i++) { 
            ...
            //Making a copy of the sub part of the array
            final int[] SUBARRAY = Arrays.copyOfRange(Array, ARRAY_START_POINT, ARRAY_START_POINT+CURRENT_SIZE);

            RunnableFuture<Integer> runnablePart = new FutureTask<>(new Callable<Integer>() {
                @Override
                public Integer call() throws Exception {
                    return Searcher.search(SUBARRAY, toSearch, ARRAY_START_POINT, THREAD_NUMBER);
                }
            });
        ...
        System.out.println("The number " + toSearch + " hasn't been found in the array :(");
        return -1;
    }
     ...
     */
    private static int search(int[] array, int toSearch, int offset, int threadNumber) {
        if (array == null)
